...
Build these projects to solidify your HTML knowledge. Start simple and progressively increase complexity.

### Project 1: Personal Portfolio Landing Page (Beginner)
**Goal**: Create a single-page portfolio with semantic HTML
...
**Skills Practiced**: Document structure, semantic HTML, forms, accessibility basics

---

### Project 2: Recipe Website (Beginner-Intermediate)
**Goal**: Multi-page website with complex content structure

**Requirements**:
- Homepage with featured recipes (use `<article>` and `<figure>`)
- Individual recipe pages with:
  - Ingredients list (ordered/unordered lists)
  - Step-by-step instructions (ordered list)
  - Cooking time metadata (use `<time>` tag)
  - Nutrition information (use `<table>`)
  - Recipe image with `<picture>` for responsive images
- About page with chef bio
- Contact page with detailed form (name, email, message, dietary preferences)
- Use `<details>` and `<summary>` for FAQ section
- Breadcrumb navigation
- Proper meta tags for SEO

**Skills Practiced**: Multi-page structure, tables, lists, forms, metadata, responsive images

---

### Project 3: Accessible Documentation Site (Intermediate)
**Goal**: Create a technical documentation site with strong accessibility

**Requirements**:
- Multi-page documentation with sidebar navigation
- Table of contents with anchor links
- Code examples using `<pre>` and `<code>` tags
- Search functionality (form with search input)
- Keyboard navigation support (proper `tabindex`)
- ARIA labels and roles where appropriate
- Skip navigation link
- Proper heading hierarchy for screen readers
- Video tutorials with `<video>` tag and captions (`<track>`)
- Download links for PDF resources
- Dark/light mode toggle (HTML structure only)
- Print-friendly structure

**Skills Practiced**: Accessibility (ARIA, tabindex), media elements, advanced navigation, semantic structure

---

### Project 4: Job Board Platform (Intermediate-Advanced)
**Goal**: Complex form-heavy application structure

**Requirements**:
- Homepage with job listings (use `<article>` for each job)
- Advanced search/filter form with:
  - Text inputs (job title, location)
  - Select dropdowns (category, experience level)
  - Checkboxes (remote, full-time, part-time)
  - Range slider (salary range)
  - Date picker (posted date)
- Job detail page with:
  - Company info (`<address>` tag)
  - Job description (semantic formatting)
  - Application form with file upload
  - Share buttons (social media links)
- Employer dashboard with:
  - Post job form (multi-step using `<fieldset>` and `<legend>`)
  - Data validation (required, pattern, min/max)
  - Rich text editor structure (`contenteditable`)
- User registration/login forms
- Email verification flow (structure only)

**Skills Practiced**: Complex forms, validation, file uploads, multi-step forms, data attributes

---

### Project 5: E-Learning Platform (Advanced)
**Goal**: Build a complete learning platform structure

**Requirements**:
- Course catalog with filtering and search
- Individual course page with:
  - Video player with controls and subtitles
  - Course syllabus (nested lists)
  - Progress tracker (`<progress>` tag)
  - Quiz/assessment forms with various input types
  - Discussion forum structure (nested comments)
- Student dashboard with:
  - Enrolled courses
  - Certificates (use `<dialog>` for modal)
  - Profile settings form
- Instructor dashboard with:
  - Course creation form (complex multi-step)
  - Student analytics (tables with data)
...
- SEO optimization:
  - Open Graph tags
  - Twitter Cards
  - Structured data (using data attributes)
  - Canonical URLs

**Skills Practiced**: All HTML concepts, complex forms, accessibility, SEO, semantic structure, media elements

---

### 💡 Project Tips

1. **Validate your HTML**: Use [W3C Validator](https://validator.w3.org/) for all projects
2. **Test accessibility**: Use browser dev tools and screen readers
3. **Check responsiveness**: Use viewport meta tag and test on different screen sizes
4. **Semantic first**: Always choose semantic tags over generic `<div>` and `<span>`
5. **Document your code**: Add comments explaining your structure choices
...
```
Project 1 (2-3 hours) → Project 2 (4-6 hours) → Project 3 (6-8 hours) → Project 4 (8-12 hours) → Project 5 (12-16 hours)
```

**Total Time**: ~40-50 hours of hands-on practice
